...
import { Box, Input, Button, Typography, Stack, IconButton, Modal } from "@mui/material";
import { Close as CloseIcon, Image as ImageIcon } from "@mui/icons-material";

import Loading from "../components/loading/Loading";
import { updateLocation } from "../axios/apis";

const EditLocation = ({ location, setEditLocationModal }) => {
    const [image, setImage] = useState(null);
    const [imagePreview, setImagePreview] = useState(location?.image_src ? 
                                            `https://realauto.limsa.uz/api/uploads/images/${location.image_src}` : null);
    ...
    const handleUpdateLocation = async () => {
        if (!name.trim() || !slug.trim() || !text.trim()) {
    ...
    return (
        <Modal open={Boolean(location)} onClose={() => setEditLocationModal(false)} sx={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Box bgcolor="#424242" p={3} borderRadius={2} boxShadow={24} width={{ xs: "90vw", sm: "60vw", md: "50vw" }}>
                <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}>
                    <Typography variant="h6" sx={{ fontWeight: "bold", color: "#F44336" }}>Joylashuvni tahrirlash</Typography>
                    <IconButton onClick={() => setEditLocationModal(false)} sx={{ color: "#fff" }}>
                        <CloseIcon />
                    </IconButton>
                </Stack>
                ...
                <Stack spacing={2} alignItems="center">
                    ...
                    <Box width="100%">
                        <Typography sx={{ fontWeight: "bold", mb: 1 }}>Joylashuv nomi</Typography>
                        <Input disableUnderline sx={{ height: "48px", width: "100%", backgroundColor: "#f5f5f5", borderRadius: 1, p: 1 }} value={name} onChange={(e) => setName(e.target.value)} placeholder="Joylashuv nomini kiriting" />
                    </Box>
                    ...
                    <Box width="100%">
                        ...
                        <Input disableUnderline 
                               ...
                               value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="Slug kiriting" />
                    </Box>
                    ...
                    <Box width="100%">
                        ...
                        <Input disableUnderline 
                               sx={{ height: "48px", width: "100%", backgroundColor: "#f5f5f5", borderRadius: 1, p: 1 }} 
                               value={text} onChange={(e) => setText(e.target.value)} placeholder="Tavsif kiriting" />
                    </Box>
                </Stack>
